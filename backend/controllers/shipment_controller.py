...
def process_incoming_reading(raw_reading: dict, db) -> dict | None:
    ...
    # Demo-friendly default: always compute route options (uses ORS/Google if configured,
    # otherwise falls back to mock data with no external calls).
    always_routes = os.getenv("ALWAYS_COMPUTE_ROUTES", "true").strip().lower() in {"1", "true", "yes", "on"}
    rerouting = None
    if always_routes or float(ai_result.get("risk_score", 0.0)) >= 0.40:

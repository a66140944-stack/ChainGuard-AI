  ...
  function handleSubmit(event) {
    ...
    try {
      ...
      const matchedUser = users.find(
        (user) => normalizeEmail(user.email) === normalizedEmail && String(user.password) === String(password)
      );

      if (!matchedUser) {
        setErrorMessage(
          "Invalid email or password. Tip: emails are case-insensitive and must match exactly (e.g. gmail.com, not gamil.com)."
        );

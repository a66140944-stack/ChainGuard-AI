...
export default function SummaryBar({ shipment }) {
  ...
  const delayMinutes =
    shipment?.backendData?.delay_minutes != null
      ? Number(shipment.backendData.delay_minutes)
      : null;

  const etaText = shipment?.backendData?.eta_text || "--:--";
  const actionText = shipment?.backendData?.action || shipment?.status || "MONITOR";

  const rerouteRecommended =
    shipment?.backendData?.rerouting?.reroute_recommended != null
      ? shipment.backendData.rerouting.reroute_recommended
      : shipment?.backendData?.rerouting?.alternate != null ||
        shipment?.backendData?.rerouting?.alternate_route != null;
  ...
  return (
    <section className="rounded-xl border border-slate-200/80 bg-white/90 p-4 shadow-sm">
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-6">
        ...
        <SummaryItem icon={AlertTriangle} label="Risk" value={`${riskScore}% (${statusAppearance.label})`} />
        ...
        <SummaryItem icon={Clock} label="Delay (min)" value={delayMinutes != null ? String(Math.max(0, Math.round(delayMinutes))) : "Not available"} />
        <SummaryItem icon={Route} label="Action" value={`${actionText}${rerouteRecommended ? " (Reroute)" : ""}`} />
      </div>
    </section>
  );
}
